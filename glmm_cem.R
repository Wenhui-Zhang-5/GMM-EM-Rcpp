...
f_glm_cem <- function(samp,g,covariates,step=1000){
  m <- ncol(samp)
  n <- nrow(samp)
  X <- covariates
  ...
  for(s in 1:step){
    ...
    ###C step
    partition <- rep(0,n)
    for(i in 1:n) partition[i] <- which.max(prob[i,])
    
    #M step
    
    for(j in 1:g){
      n1 <- nrow(samp[partition==j,])
      pie[j] <- n1/n
      W <-prob[partition==j,j]
      
      for(w in 1:m) {
        fit <- glm(samp[partition==j,w]~X[[w]][partition==j,-1],family=binomial,weights =W)
    ...
    BIC <- -2*logdata+((k+1)*m*g+g-1)*log(n)
...
r <- f_glm_cem(samp=d$measurement,g=2,covariates=d$covariates)
r$s
system.time(r <- f_glm(samp=d$measurement,g=2,covariates=d$covariates))
r$label
r$pie
table(r$label)
la <- c(rep(2,400),rep(1,600))
table(la,r$label)
...
r1 <- f_glm_cem(samp=d1$measurement,g=3,covariates=d1$covariates)
